...
router.post('/v28/where-do-you-live', function (req, res) {


var location = req.session.data['location']

if (location === "england") {
  res.redirect('/v28/apply/name')
}
if (location === "wales") {
  res.redirect('/v28/apply/name')
}
...
if (location === "somewhere else") {
  res.redirect('/v28/apply/kickouts/not-eligible-country')
}
...
router.post('/v28/name', function (req, res) {

    var firstname = req.session.data['firstname']
    var lastname = req.session.data['lastname']

    res.redirect('/v28/apply/postcode')
  ...
  router.post('/v28/apply/postcode', function (req, res) {

    res.redirect('/v28/apply/address-2');
  ...
  router.post('/v28/apply/address-2', function (req, res) {
    ...
    res.redirect('/v28/apply/date-of-birth');
});
  ...
  router.post('/v28/date-of-birth', function (req, res) {
  ...
  if (yrs < 16) {
      res.redirect('/v28/apply/kickouts/confirmation-under-16');
  } else {
      res.redirect('/v28/apply/national-insurance-number');
  }
});
  ...
  router.post('/v28/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');

    res.redirect('/v28/apply/check-your-answers-personal-details')
    ...
    router.post('/v28/cya-personal-details', (req, res) => {
    //res.render('/v28/cya-personal-details', { data: req.session.data }); //Ensure Data is Properly Passed (it works without this)
    ...
    var yrs = moment().diff(moment(dobYrs), 'years'); // Calculate age
      ...
      if (firstname == 'RILEY' && lastname == 'JONES' && nationalinsurancenumber == 'CD654321B' && dateofbirth == '01/01/2000') {
        res.redirect('/v28/apply/kickouts/no-api-match-benefit')
      }
      else {
        res.redirect('/v28/apply/are-you-pregnant')
      }
  ...
  router.post('/v28/are-you-pregnant', function (req, res) {

    var pregnant = req.session.data['pregnant']

    if (pregnant === "yes") {
      res.redirect('/v28/apply/due-date')
    }
    else if (pregnant === "no") {
      req.session.data.lessThanTenWeeksPregnant = true;
      res.redirect('/v28/apply/children-under-four')
    }
    else {
      res.redirect('/v28/apply/are-you-pregnant')
    }
  ...
  router.post('/v28/due-date', function (req, res) {
  ...
  router.post('/v28/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v28/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v28/apply/childs-first-name')
    }
    ...
    router.post('/v28/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v28/apply/children-under-four-answers');
    ...
    router.post('/v28/children-under-four-answers', function (req, res) {
  ...
  router.post('/v28/email-address', function (req, res) {

    var emailaddress = req.session.data['emailaddress']

    res.redirect('/v28/apply/mobile-phone-number')

  })

  // What is your mobile phone number?

  router.post('/v28/mobile-phone-number', function (req, res) {

    var mobilePhoneNumber = req.session.data['mobilephonenumber']

    res.redirect('/v28/apply/check-your-answers')
...
router.post('/v28/check-your-answers', function (req, res) {
  ...
  res.redirect('/v28/apply/confirmation-successful');
})
