...
router.post('/v30/where-do-you-live', function (req, res) {


var location = req.session.data['location']

if (location === "england") {
  res.redirect('/v30/apply/name')
}
if (location === "wales") {
  res.redirect('/v30/apply/name')
}
...
if (location === "somewhere else") {
  res.redirect('/v30/apply/kickouts/not-eligible-country')
}
...
router.post('/v30/name', function (req, res) {

    var firstname = req.session.data['firstname']
    var lastname = req.session.data['lastname']

    res.redirect('/v30/apply/postcode')
  ...
  router.post('/v30/apply/postcode', function (req, res) {

    res.redirect('/v30/apply/address-2');
  ...
  router.post('/v30/apply/address-2', function (req, res) {
    ...
    res.redirect('/v30/apply/date-of-birth');
});
  ...
  router.post('/v30/date-of-birth', function (req, res) {
  ...
  if (yrs < 16) {
      res.redirect('/v30/apply/kickouts/confirmation-under-16');
  } else {
      res.redirect('/v30/apply/national-insurance-number');
  }
});
  ...
  router.post('/v30/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');

    res.redirect('/v30/apply/check-your-answers-personal-details')
    ...
    router.post('/v30/cya-personal-details', (req, res) => {
    //res.render('/v30/cya-personal-details', { data: req.session.data }); //Ensure Data is Properly Passed (it works without this)
    ...
    var yrs = moment().diff(moment(dobYrs), 'years'); // Calculate age
      ...
      if (firstname == 'RILEY' && lastname == 'JONES' && nationalinsurancenumber == 'CD654321B' && dateofbirth == '01/01/2000') {
        res.redirect('/v30/apply/kickouts/no-api-match-benefit')
      }
      else {
        res.redirect('/v30/apply/are-you-pregnant')
      }
  ...
  router.post('/v30/are-you-pregnant', function (req, res) {
    ...
    if (pregnant === "yes") {
      res.redirect('/v30/apply/due-date');
    }
    else if (pregnant === "no") {

      // NEW LOGIC: 16–17 year olds who are not pregnant
      if (yrs === 16 || yrs === 17) {
        return res.redirect('/v30/apply/kickouts/16-17-year-old-not-pregnant');
      }

      req.session.data.lessThanTenWeeksPregnant = true;
      res.redirect('/v30/apply/children-under-four');
    }
    else {
      res.redirect('/v30/apply/are-you-pregnant');
    }
  ...
  router.post('/v30/due-date', function (req, res) {
  ...
  router.post('/v30/children-under-four', function (req, res) {
    ...
    if (yrs >= 16 && yrs <= 17 && pregnant === "yes" && childrenunderfour === "yes") {
      return res.redirect('/v30/apply/16-17-cannot-receive-child-payments');
    }

    if (yrs >= 16 && yrs <= 17 && pregnant === "yes" && childrenunderfour === "no") {
      return res.redirect('/v30/apply/bank-details');
    }

    // Existing logic
    if (pregnant === "yes" && childrenunderfour === "no") {
      res.redirect('/v30/apply/email-address');
    } else if (pregnant === "no" && childrenunderfour === "yes") {
      res.redirect('/v30/apply/childs-first-name');
    } else if (pregnant === "yes" && childrenunderfour === "yes") {
      res.redirect('/v30/apply/childs-first-name');
    } else if (childrenunderfour === "no" && pregnant === "no") {
      res.redirect('/v30/apply/kickouts/not-pregnant-no-children');
    } else {
      res.redirect('/v30/apply/children-under-four');
    }
  ...
  router.post('/v30/bank-details', function (req, res) {

    res.redirect('/v30/apply/identity-evidence')
  ...
  router.post('/v30/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v30/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v30/apply/childs-first-name')
    }
    ...
    router.post('/v30/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v30/apply/children-under-four-answers');
    ...
    router.post('/v30/children-under-four-answers', function (req, res) {
  ...
  router.post('/v30/email-address', function (req, res) {

    const noEmail = req.session.data['no-email']

    if (Array.isArray(noEmail) && noEmail.includes('true')) {
      req.session.data['contactMethod'] = 'text';
      res.redirect('/v30/apply/mobile-phone-number')
    } else {
      req.session.data['contactMethod'] = 'email';
      res.redirect('/v30/apply/marketing-preference')
    }

  })

  // How do you want to be contacted by Healthy Start?

  router.post('/v30/contact-method', function (req, res) {
  ...
  router.post('/v30/mobile-phone-number', function (req, res) {

    var mobilePhoneNumber = req.session.data['mobilephonenumber']

    res.redirect('/v30/apply/check-your-answers')

  })

  // Do you consent to additional email communications?

  router.post('/v30/marketing-preference', function (req, res) {

    var additionalcomms = req.session.data['additional-comms']

    if (additionalcomms === "no") {
      res.redirect('/v30/apply/mobile-phone-number')
    } else {
      res.redirect('/v30/apply/marketing-preference-options')
    }
  ...
  router.post('/v30/marketing-preference-options', function (req, res) {
...
router.post('/v30/check-your-answers', function (req, res) {
  ...
  if (age === 16 || age === 17) {
    return res.redirect('/v30/apply/confirmation-submitted');
  } else {
  res.redirect('/v30/apply/confirmation-successful');
  }
})
