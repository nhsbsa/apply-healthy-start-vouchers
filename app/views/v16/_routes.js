...
router.post('/v16/where-do-you-live', function (req, res) {

  const whereDoYouLive = req.session.data['where-do-you-live']

  if (whereDoYouLive == 'yes') {
    res.redirect('/v16/apply/name');
  } else {
    res.redirect('/v16/apply/kickouts/not-eligible-country');
  }
...
router.post('/v16/name', function (req, res) {
    ...
    if (firstname && lastname) {
      res.redirect('/v16/apply/address')
    }
    else {
      res.redirect('/v16/apply/name')
    }
  ...
  router.get('/v16/find-address', function (req, res) {
    ...
    if (regex.test(postcode) === true) {
      ...
      if (houseNumberName) {
        ...
        .then(response => {
          ...
          res.render('v16/apply/select-address', {items: items});
        })
        .catch(error => {
          console.log(error);
          res.redirect('/v16/apply/no-address-found')
        });
      ...
      res.redirect('/v16/apply/find-address')
    }
  ...
  router.get('/v16/select-address', function (req, res) {
    ...
    if (selectaddress === 'none') {
      ...
      res.redirect('/v16/apply/date-of-birth')
    } else {
      res.redirect('/v16/apply/select-address')
    }
  ...
  router.post('/v16/address', function (req, res) {
  ...
  req.session.data['dateofbirth'] = new Intl.DateTimeFormat('en-GB', {year: 'numeric', month: 'long', day: 'numeric'}).format(new Date(dateofbirth))
    ...
    if (dateofbirthday && dateofbirthmonth && dateofbirthyear) {
      if (yrs < 16) {
        res.redirect('/v16/apply/kickouts/under-sixteen-signpost')
      } else {
  ...
  router.post('/v16/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');
  
    if (nationalinsurancenumber) {
      res.redirect('/v16/apply/check-your-answers-personal-details')
    } else {
      res.redirect('/v16/apply/national-insurance-number')
    }
  ...
  router.post('/v16/nationality', function (req, res) {
    ...
    if (nationality) {
      ...
      res.redirect('/v16/apply/nationality')
    }
  ...
  router.post('/v16/are-you-pregnant', function (req, res) {
  
    var pregnant = req.session.data['pregnant']
  
    if (pregnant === "yes") {
      res.redirect('/v16/apply/due-date')
    }
    else if (pregnant === "no") {
      req.session.data.lessThanTenWeeksPregnant = true;
      res.redirect('/v16/apply/children-under-four')
    }
    else {
      res.redirect('/v16/apply/are-you-pregnant')
    }
  ...
  router.post('/v16/due-date', function (req, res) {
    ...
    if (duedateday && duedatemonth && duedateyear) {
      ...
      if (duedate < today) {
        ...
        res.redirect('/v16/apply/email-address')
      }
  ...
  router.post('/v16/children-under-four', function (req, res) {
    ...
    if (pregnant === "yes" && childrenunderfour === "no") {
      ...
      res.redirect('/v16/apply/childs-first-name')
    } else if (pregnant === "yes" && childrenunderfour === "yes") {
      res.redirect('/v16/apply/childs-first-name')
    } else if (childrenunderfour === "no" && pregnant ==="no") {
      res.redirect('/v16/apply/kickouts/not-eligible')
    } else {
      res.redirect('/v16/apply/children-under-four')
    }
  ...
  router.post('/v16/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v16/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v16/apply/childs-first-name')
    }
    ...
    router.post('/v16/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v16/apply/children-under-four-answers'); 
    ...
    router.post('/v16/children-under-four-answers', function (req, res) {
      ...
      if (childrenunderfouranswers === "yes") {
        ...
        res.redirect('/v16/apply/kickouts/no-eligible-children')
      }
  ...
  router.post('/v16/email-address', function (req, res) {
  
    var emailaddress = req.session.data['emailaddress']
  
    res.redirect('/v16/apply/mobile-phone-number')
  
  })
  
  // What is your mobile phone number?
  
  router.post('/v16/mobile-phone-number', function (req, res) {
  
    var mobilePhoneNumber = req.session.data['mobilephonenumber']
  
    res.redirect('/v16/apply/check-your-answers')
  
  })
  
  // Contact Preferences
  
  router.post('/v16/contact-preferences', function (req, res) {
    ...
    if (contact) {
      ...
      if (emailAddress || mobile || contact === 'NONE'){
        res.redirect('/v16/apply/bank-details')    
      }
      else {
        res.redirect('/v16/apply/contact-preferences')
      }
  ...
  router.post('/v16/feedback', function (req, res) {
    res.redirect('/v16/feedback')
  })


// v16 Check Your Answers
...
router.post('/v16/check-your-answers', function (req, res) {
  ...
  if (lastname == 'Green') {
    res.redirect('/v16/apply/confirmation-pending-evidence')
    ...
    if (emailAddress) {
      ...
      if (pregnant === "yes") {
            res.redirect('/v16/apply/confirmation-successful');
      } else {
        res.redirect('/v16/apply/confirmation-successful');
      }
