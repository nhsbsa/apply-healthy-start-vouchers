...
router.post('/v22/where-do-you-live', function (req, res) {

  const whereDoYouLive = req.session.data['where-do-you-live']

  if (whereDoYouLive == 'yes') {
    res.redirect('/v22/apply/date-of-birth');
  } else {
    res.redirect('/v22/apply/kickouts/not-eligible-country');
  }
...
router.post('/v22/date-of-birth', function (req, res) {
  ...
  if (dateofbirthday && dateofbirthmonth && dateofbirthyear) {
    if (yrs < 16) {
      res.redirect('/v22/apply/kickouts/under-sixteen-signpost')
    } else {
      res.redirect('/v22/apply/name');
    }
  } else {
    res.redirect('/v22/apply/date-of-birth')
  }
...
router.post('/v22/name', function (req, res) {

  var firstname = req.session.data['firstname']
  var lastname = req.session.data['lastname']

  if (firstname && lastname) {
    res.redirect('/v22/apply/address')
  }
  else {
    res.redirect('/v22/apply/name')
  }
  ...
  router.post('/v22/address', function (req, res) {
    ...
    if (addressline1 && towncity && postcode) {
      res.redirect('/v22/apply/national-insurance-number')
    } else {
      res.redirect('/v22/apply/address')
    }
  ...
  router.post('/v22/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');
  
    if (nationalinsurancenumber) {
      res.redirect('/v22/apply/check-your-answers-personal-details')
    } else {
      res.redirect('/v22/apply/national-insurance-number')
    }
  ...
  router.post('/v22/cya-personal-details', (req, res) => {
  ...
  req.session.data['dateofbirth'] = new Intl.DateTimeFormat('en-GB', {year: 'numeric', month: 'long', day: 'numeric'}).format(new Date(dateofbirth))
    ...
    if (nationalinsurancenumber) {
      ...
      if (firstname == 'CHARLIE' && lastname == 'SMITH' && nationalinsurancenumber == 'AB123456A' && dateofbirth == '01/01/2000' && postcode == 'LL673SN' && addressline1 == '55 PEACHFIELD ROAD') {
        res.redirect('/v22/apply/are-you-pregnant')
        ...
        res.redirect('/v22/apply/kickouts/confirmation-no-match')
        ...
        if (yrs >= 16) {
          res.redirect('/v22/apply/benefits')
        }
      } else {
        res.redirect('/v22/apply/kickouts/confirmation-no-match')
      }  
...
router.post('/v22/benefits', function (req, res) {
    ...
    if (Array.isArray(benefits)) {
      ...
      if (benefits.includes('UC')) {
        if (fullName == "SARAH GREEN") {
          res.redirect('/v22/apply/universal-credit')
        } else {
          res.redirect('/v22/apply/are-you-pregnant')
        }
        ...
        res.redirect('/v22/apply/jobseekers-allowance')
      } else if (benefits.includes('ESA' && 'WTC') && !benefits.includes('JSA') && !benefits.includes('IS')) {
        res.redirect('/v22/apply/working-tax-credits')
        ...
        if (benefits.includes('UC')) {
          ...
          if (fullName == "SARAH GREEN") {
            res.redirect('/v22/apply/child-tax-credits')
          } else {
            res.redirect('/v22/apply/are-you-pregnant')
          }
...
router.post('/v22/universal-credit', function (req, res) {
  ...
  if (universalcredit === "yes") {
    ...
    res.redirect('/v22/apply/kickouts/not-eligible')
  }
  else {
    res.redirect('/v22/apply/universal-credit')
  }
...
router.post('/v22/child-tax-credits', function (req, res) {
...
router.get('/v22/jobseekers-allowance', (req, res) => {
...
router.get('/v22/employment-support-allowance', (req, res) => {
...
router.get('/v22/working-tax-credits', (req, res) => {
...
router.post('/v22/pension-credit', function (req, res) {
  ...
  router.post('/v22/nationality', function (req, res) {
    ...
    if (nationality) {
      ...
      res.redirect('/v22/apply/nationality')
    }
  ...
  router.post('/v22/are-you-pregnant', function (req, res) {
    ...
    if (pregnant === "yes") {
      res.redirect('/v22/apply/due-date')
    }
    else if (pregnant === "no") {
      ...
      if (benefits === 'ESA' && typeOfESA === 'income') {
        ...
        res.redirect('/v22/apply/children-under-four')
      }
  ...
  router.post('/v22/due-date', function (req, res) {
  ...
  router.post('/v22/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v22/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v22/apply/childs-first-name')
    }
    ...
    router.post('/v22/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v22/apply/children-under-four-answers'); 
    ...
    router.post('/v22/children-under-four-answers', function (req, res) {
      ...
      if (childrenunderfouranswers === "yes") {
        ...
        res.redirect('/v22/apply/kickouts/no-eligible-children')
      }
  ...
  router.post('/v22/email-address', function (req, res) {
  
    var emailaddress = req.session.data['emailaddress']
  
    res.redirect('/v22/apply/mobile-phone-number')
  
  })
  
  // What is your mobile phone number?
  
  router.post('/v22/mobile-phone-number', function (req, res) {
    ...
    res.redirect('/v22/apply/check-your-answers')
  
  })
  
  // Contact Preferences
  
  router.post('/v22/contact-preferences', function (req, res) {
    ...
    if (contact) {
      ...
      if (emailAddress || mobile || contact === 'NONE'){
        res.redirect('/v22/apply/bank-details')    
      }
      else {
        res.redirect('/v22/apply/contact-preferences')
      }
  ...
  router.post('/v22/bank-details', function (req, res) {
  ...
  router.post('/v22/feedback', function (req, res) {
    res.redirect('/v22/feedback')
  })


// v22 Check Your Answers

// N.B ALL PERSONALISATION VARIABLES NEED TO BE THERE, IF THEY'RE NOT REQUIRED YOU STILL NEED TO SEND AN EMPTY STRING ""

router.post('/v22/check-your-answers', function (req, res) {
  ...
  if (lastname == 'Green' || lastname == 'Elba') {
    res.redirect('/v22/apply/confirmation-pending-evidence')
    ...
    if (emailAddress) {
      ...
      if (pregnant === "yes") {
            res.redirect('/v22/apply/confirmation-successful');
      } else {
        res.redirect('/v22/apply/confirmation-successful');
      }
...
router.post('/v22/return-reference-number', function (req, res) {

  var referencenumber = req.session.data['referencenumber']
  req.session.data['savedUntil'] = moment().add(3, 'months').format("D MMMM YYYY")

  if (referencenumber) {
    res.redirect('/v22/apply/return-date-of-birth')
  }
  else {
    res.redirect('/v22/apply/return-reference-number')
  }
...
router.post('/v22/return-date-of-birth', function (req, res) {
  ...
  if (returnday && returnmonth && returnyear) {
    res.redirect('/v22/apply/return-task-list')
  }
  else {
    res.redirect('/v22/apply/return-date-of-birth')
  }
...
router.post('/v22/return-applicant-identity-select', function (req, res) {

  var applicantidentity = req.session.data['applicantidentity']

  if (applicantidentity) {
    res.redirect('/v22/apply/return-applicant-identity')
  }
  else {
    res.redirect('/v22/apply/return-applicant-identity')
  }
...
router.post('/v22/return-applicant-identity-upload', function (req, res) {
  ...
  if (anotherFile === 'yes') {
    ...
    res.redirect('/v22/apply/return-applicant-identity-upload')
  }
...
router.post('/v22/return-applicant-eligibility-upload', function (req, res) {

  const anotherFile = req.session.data['add-file2']

  if (anotherFile === 'yes') {
    res.redirect('/v22/apply/return-applicant-eligibility')
  } else if (anotherFile === 'no') {
    req.session.data["applicant-eligibility"] = "Complete";
    res.redirect('/v22/apply/return-task-list')
  } else {
    res.redirect('/v22/apply/return-applicant-eligibility-upload')
  }
...
router.post('/v22/return-child-eligibility-upload', function (req, res) {

  const anotherFile = req.session.data['add-file3']

  if (anotherFile === 'yes') {
    res.redirect('/v22/apply/return-child-eligibility')
  } else if (anotherFile === 'no') {
    req.session.data["child-eligibility"] = "Complete";
    res.redirect('/v22/apply/return-task-list')
  } else {
    res.redirect('/v22/apply/return-child-eligibility-upload')
  }
...
router.post('/v22/online-account', function (req, res) {
  ...
  if (onlineAccount) {
    ...
    res.redirect('/v22/apply/online-account')
  }
...
router.post('/v22/email-address-2', function (req, res) {
