...
router.post('/v20/where-do-you-live', function (req, res) {

  const whereDoYouLive = req.session.data['where-do-you-live']

  if (whereDoYouLive == 'yes') {
    res.redirect('/v20/apply/date-of-birth');
  } else {
    res.redirect('/v20/apply/kickouts/not-eligible-country');
  }
...
router.post('/v20/date-of-birth', function (req, res) {
  ...
  if (dateofbirthday && dateofbirthmonth && dateofbirthyear) {
    if (yrs < 16) {
      res.redirect('/v20/apply/kickouts/under-sixteen-signpost')
    } else {
      res.redirect('/v20/apply/name');
    }
  } else {
    res.redirect('/v20/apply/date-of-birth')
  }
...
router.post('/v20/name', function (req, res) {
  ...
  if (firstname && lastname) {
    res.redirect('/v20/apply/postcode')
  }
  else {
    res.redirect('/v20/apply/name')
  }
  ...
  router.post('/v20/address', function (req, res) {
    ...
    if (addressline1 && towncity && postcode) {
      res.redirect('/v20/apply/national-insurance-number')
    } else {
      res.redirect('/v20/apply/address')
    }
  ...
  router.post('/v20/postcode', function (req, res) {

    res.redirect('/v20/apply/address-2');
  
  })

  // What is your address 2?

  router.post('/v20/apply/address-2', function (req, res) {
  ...
  router.post('/v20/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');
  
    if (nationalinsurancenumber) {
      res.redirect('/v20/apply/check-your-answers-personal-details')
    } else {
      res.redirect('/v20/apply/national-insurance-number')
    }
  ...
  router.post('/v20/cya-personal-details', (req, res) => {
  ...
  req.session.data['dateofbirth'] = new Intl.DateTimeFormat('en-GB', {year: 'numeric', month: 'long', day: 'numeric'}).format(new Date(dateofbirth))
    ...
    if (nationalinsurancenumber) {
      ...
      if (firstname == 'CHARLIE' && lastname == 'SMITH' && nationalinsurancenumber == 'AB123456A' && dateofbirth == '01/01/2000' && postcode == 'LL673SN' && addressline1 == '55 PEACHFIELD ROAD') {
        ...
        res.redirect('/v20/apply/kickouts/confirmation-no-match')
  ...
  router.post('/v20/benefits', function (req, res) {
      ...
      if (Array.isArray(benefits)) {
        ...
        if (benefits.includes('UC')) {
          if (fullName == "SARAH GREEN") {
            res.redirect('/v20/apply/universal-credit')
          } else {
            res.redirect('/v20/apply/universal-credit')
          }
          ...
          res.redirect('/v20/apply/jobseekers-allowance')
        } else if (benefits.includes('ESA' && 'WTC') && !benefits.includes('JSA') && !benefits.includes('IS')) {
          res.redirect('/v20/apply/working-tax-credits')
          ...
          if (benefits.includes('UC')) {
            ...
            if (fullName == "SARAH GREEN") {
              res.redirect('/v20/apply/child-tax-credits')
            } else {
              res.redirect('/v20/apply/child-tax-credits')
            }
            ...
            res.redirect('/v20/apply/employment-support-allowance')
          } else if (benefits.includes('IS')) {
            res.redirect('/v20/apply/are-you-pregnant')
          } else if (benefits.includes('PC')) {
            res.redirect('/v20/apply/pension-credit')
          } else if (benefits.includes('WTC')) {
            res.redirect('/v20/apply/working-tax-credits')
          } else if (benefits.includes('NONE')) {
            if (applicantsAge <= 16 && applicantsAge >= 18) {
              res.redirect('/v20/apply/kickouts/confirmation-no-pregnancy-no-children')
            } else {
              res.redirect('/v20/apply/kickouts/confirmation-no-pregnancy-no-children')
            }
          } else {
...
router.post('/v20/universal-credit', function (req, res) {
...
router.post('/v20/child-tax-credits', function (req, res) {

  var childtaxcredits = req.session.data['childtaxcredits']

  if (childtaxcredits === "yes") {
    res.redirect('/v20/apply/working-tax-credit')
  }
...
router.post('/v20/working-tax-credit', function (req, res) {
...
router.get('/v20/jobseekers-allowance', (req, res) => {
...
router.get('/v20/employment-support-allowance', (req, res) => {
...
router.get('/v20/working-tax-credits', (req, res) => {
...
router.post('/v20/pension-credit', function (req, res) {
  ...
  router.post('/v20/nationality', function (req, res) {
    ...
    if (nationality) {
      ...
      res.redirect('/v20/apply/nationality')
    }
  ...
  router.post('/v20/are-you-pregnant', function (req, res) {
  ...
  router.post('/v20/due-date', function (req, res) {
  ...
  router.post('/v20/children-under-four', function (req, res) {
    ...
    if (lessThan10WeeksPregnant == true && childrenunderfour == 'no') {
      ...
      res.redirect('/v20/apply/email-address')
    } else if (pregnant === "no" && childrenunderfour === "yes") {
      res.redirect('/v20/apply/childs-first-name')
  ...
  router.post('/v20/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v20/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v20/apply/childs-first-name')
    }
    ...
    router.post('/v20/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v20/apply/children-under-four-answers'); 
    ...
    router.post('/v20/children-under-four-answers', function (req, res) {
      ...
      if (childrenunderfouranswers === "yes") {
        ...
        res.redirect('/v20/apply/kickouts/no-eligible-children')
      }
  ...
  router.post('/v20/email-address', function (req, res) {
  
    var emailaddress = req.session.data['emailaddress']
  
    res.redirect('/v20/apply/check-your-answers')
  
  })
  
  // What is your mobile phone number?
  
  router.post('/v20/mobile-phone-number', function (req, res) {
  ...
  router.post('/v20/contact-preferences', function (req, res) {
    ...
    if (contact) {
      ...
      if (emailAddress || mobile || contact === 'NONE'){
        res.redirect('/v20/apply/bank-details')    
      }
      else {
        res.redirect('/v20/apply/contact-preferences')
      }
  ...
  router.post('/v20/bank-details', function (req, res) {
  ...
  router.post('/v20/feedback', function (req, res) {
    res.redirect('/v20/feedback')
  })


// v20 Check Your Answers
...
router.post('/v20/check-your-answers', function (req, res) {
  ...
  if (lastname == 'Green' || lastname == 'Elba') {
    res.redirect('/v20/apply/confirmation-pending-evidence')
    ...
    if (emailAddress) {
      ...
      if (pregnant === "yes") {
            res.redirect('/v20/apply/confirmation-successful');
      } else {
        res.redirect('/v20/apply/confirmation-successful');
      }
...
router.post('/v20/return-reference-number', function (req, res) {

  var referencenumber = req.session.data['referencenumber']
  req.session.data['savedUntil'] = moment().add(3, 'months').format("D MMMM YYYY")

  if (referencenumber) {
    res.redirect('/v20/apply/return-date-of-birth')
  }
  else {
    res.redirect('/v20/apply/return-reference-number')
  }
...
router.post('/v20/return-date-of-birth', function (req, res) {
  ...
  if (returnday && returnmonth && returnyear) {
    res.redirect('/v20/apply/return-task-list')
  }
  else {
    res.redirect('/v20/apply/return-date-of-birth')
  }
...
router.post('/v20/return-applicant-identity-select', function (req, res) {

  var applicantidentity = req.session.data['applicantidentity']

  if (applicantidentity) {
    res.redirect('/v20/apply/return-applicant-identity')
  }
  else {
    res.redirect('/v20/apply/return-applicant-identity')
  }
...
router.post('/v20/return-applicant-identity-upload', function (req, res) {
  ...
  if (anotherFile === 'yes') {
    ...
    res.redirect('/v20/apply/return-applicant-identity-upload')
  }
...
router.post('/v20/return-applicant-eligibility-upload', function (req, res) {

  const anotherFile = req.session.data['add-file2']

  if (anotherFile === 'yes') {
    res.redirect('/v20/apply/return-applicant-eligibility')
  } else if (anotherFile === 'no') {
    req.session.data["applicant-eligibility"] = "Complete";
    res.redirect('/v20/apply/return-task-list')
  } else {
    res.redirect('/v20/apply/return-applicant-eligibility-upload')
  }
...
router.post('/v20/return-child-eligibility-upload', function (req, res) {

  const anotherFile = req.session.data['add-file3']

  if (anotherFile === 'yes') {
    res.redirect('/v20/apply/return-child-eligibility')
  } else if (anotherFile === 'no') {
    req.session.data["child-eligibility"] = "Complete";
    res.redirect('/v20/apply/return-task-list')
  } else {
    res.redirect('/v20/apply/return-child-eligibility-upload')
  }
...
router.post('/v20/online-account', function (req, res) {
  ...
  if (onlineAccount) {
    ...
    res.redirect('/v20/apply/online-account')
  }
...
router.post('/v20/email-address-2', function (req, res) {

  var emailaddress = req.session.data['emailaddress']

  res.redirect('/v20/apply/mobile-phone-number')
