...
router.post('/v29-u18/where-do-you-live', function (req, res) {


var location = req.session.data['location']

if (location === "england") {
  res.redirect('/v29-u18/apply/name')
}
if (location === "wales") {
  res.redirect('/v29-u18/apply/name')
}
...
if (location === "somewhere else") {
  res.redirect('/v29-u18/apply/kickouts/not-eligible-country')
}
...
router.post('/v29-u18/name', function (req, res) {

    var firstname = req.session.data['firstname']
    var lastname = req.session.data['lastname']

    res.redirect('/v29-u18/apply/postcode')
  ...
  router.post('/v29-u18/apply/postcode', function (req, res) {

    res.redirect('/v29-u18/apply/address-2');
  ...
  router.post('/v29-u18/apply/address-2', function (req, res) {
    ...
    res.redirect('/v29-u18/apply/date-of-birth');
});
  ...
  router.post('/v29-u18/date-of-birth', function (req, res) {
  ...
  if (yrs < 16) {
      res.redirect('/v29-u18/apply/kickouts/confirmation-under-16');
  } else {
      res.redirect('/v29-u18/apply/national-insurance-number');
  }
});
  ...
  router.post('/v29-u18/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');

    res.redirect('/v29-u18/apply/check-your-answers-personal-details')
    ...
    router.post('/v29-u18/cya-personal-details', (req, res) => {
    //res.render('/v29-u18/cya-personal-details', { data: req.session.data }); //Ensure Data is Properly Passed (it works without this)
    ...
    var yrs = moment().diff(moment(dobYrs), 'years'); // Calculate age
      ...
      if (firstname == 'RILEY' && lastname == 'JONES' && nationalinsurancenumber == 'CD654321B' && dateofbirth == '01/01/2000') {
        res.redirect('/v29-u18/apply/kickouts/no-api-match-benefit')
      }
      else {
  ...
  router.post('/v29-u18/due-date', function (req, res) {
  ...
  router.post('/v29-u18/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v29-u18/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v29-u18/apply/childs-first-name')
    }
    ...
    router.post('/v29-u18/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v29-u18/apply/children-under-four-answers');
    ...
    router.post('/v29-u18/children-under-four-answers', function (req, res) {
  ...
  router.post('/v29-u18/email-address', function (req, res) {

    var emailaddress = req.session.data['emailaddress']

    res.redirect('/v29-u18/apply/mobile-phone-number')

  })

  // What is your mobile phone number?

  router.post('/v29-u18/mobile-phone-number', function (req, res) {

    var mobilePhoneNumber = req.session.data['mobilephonenumber']

    res.redirect('/v29-u18/apply/check-your-answers')
