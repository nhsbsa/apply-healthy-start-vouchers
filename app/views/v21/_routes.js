...
router.post('/v21/where-do-you-live', function (req, res) {

  const whereDoYouLive = req.session.data['where-do-you-live']

  if (whereDoYouLive == 'yes') {
    res.redirect('/v21/apply/date-of-birth');
  } else {
    res.redirect('/v21/apply/kickouts/not-eligible-country');
  }
...
router.post('/v21/date-of-birth', function (req, res) {
  ...
  if (dateofbirthday && dateofbirthmonth && dateofbirthyear) {
    if (yrs < 16) {
      res.redirect('/v21/apply/kickouts/under-sixteen-signpost')
    } else {
      res.redirect('/v21/apply/name');
    }
  } else {
    res.redirect('/v21/apply/date-of-birth')
  }
...
router.post('/v21/name', function (req, res) {

  var firstname = req.session.data['firstname']
  var lastname = req.session.data['lastname']

  if (firstname && lastname) {
    res.redirect('/v21/apply/address')
  }
  else {
    res.redirect('/v21/apply/name')
  }
  ...
  router.post('/v21/address', function (req, res) {
    ...
    if (addressline1 && towncity && postcode) {
      res.redirect('/v21/apply/national-insurance-number')
    } else {
      res.redirect('/v21/apply/address')
    }
  ...
  router.post('/v21/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');
  
    if (nationalinsurancenumber) {
      res.redirect('/v21/apply/check-your-answers-personal-details')
    } else {
      res.redirect('/v21/apply/national-insurance-number')
    }
  ...
  router.post('/v21/cya-personal-details', (req, res) => {
  ...
  req.session.data['dateofbirth'] = new Intl.DateTimeFormat('en-GB', {year: 'numeric', month: 'long', day: 'numeric'}).format(new Date(dateofbirth))
    ...
    if (nationalinsurancenumber) {
      ...
      if (firstname == 'CHARLIE' && lastname == 'SMITH' && nationalinsurancenumber == 'AB123456A' && dateofbirth == '01/01/2000' && postcode == 'LL673SN' && addressline1 == '55 PEACHFIELD ROAD') {
        res.redirect('/v21/apply/are-you-pregnant')
        ...
        res.redirect('/v21/apply/kickouts/confirmation-no-match')
        ...
        if (yrs >= 16) {
          res.redirect('/v21/apply/benefits')
        }
      } else {
        res.redirect('/v21/apply/kickouts/confirmation-no-match')
      }  
...
router.post('/v21/benefits', function (req, res) {
    ...
    if (Array.isArray(benefits)) {
      ...
      if (benefits.includes('UC')) {
        if (fullName == "SARAH GREEN") {
          res.redirect('/v21/apply/universal-credit')
        } else {
          res.redirect('/v21/apply/are-you-pregnant')
        }
        ...
        res.redirect('/v21/apply/jobseekers-allowance')
      } else if (benefits.includes('ESA' && 'WTC') && !benefits.includes('JSA') && !benefits.includes('IS')) {
        res.redirect('/v21/apply/working-tax-credits')
        ...
        if (benefits.includes('UC')) {
          ...
          if (fullName == "SARAH GREEN") {
            res.redirect('/v21/apply/child-tax-credits')
          } else {
            res.redirect('/v21/apply/are-you-pregnant')
          }
...
router.post('/v21/universal-credit', function (req, res) {
...
router.post('/v21/child-tax-credits', function (req, res) {
...
router.get('/v21/jobseekers-allowance', (req, res) => {
...
router.get('/v21/employment-support-allowance', (req, res) => {
...
router.get('/v21/working-tax-credits', (req, res) => {
...
router.post('/v21/pension-credit', function (req, res) {
  ...
  router.post('/v21/nationality', function (req, res) {
    ...
    if (nationality) {
      ...
      res.redirect('/v21/apply/nationality')
    }
  ...
  router.post('/v21/are-you-pregnant', function (req, res) {
    ...
    if (pregnant === "yes") {
      res.redirect('/v21/apply/due-date')
    }
    else if (pregnant === "no") {
  ...
  router.post('/v21/due-date', function (req, res) {
  ...
  router.post('/v21/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v21/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v21/apply/childs-first-name')
    }
    ...
    router.post('/v21/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v21/apply/children-under-four-answers'); 
    ...
    router.post('/v21/children-under-four-answers', function (req, res) {
      ...
      if (childrenunderfouranswers === "yes") {
        ...
        res.redirect('/v21/apply/kickouts/no-eligible-children')
      }
  ...
  router.post('/v21/email-address', function (req, res) {
  
    var emailaddress = req.session.data['emailaddress']
  
    res.redirect('/v21/apply/mobile-phone-number')
  ...
  router.post('/v21/contact-preferences', function (req, res) {
    ...
    if (contact) {
      ...
      if (emailAddress || mobile || contact === 'NONE'){
        res.redirect('/v21/apply/bank-details')    
      }
      else {
        res.redirect('/v21/apply/contact-preferences')
      }
  ...
  router.post('/v21/bank-details', function (req, res) {
  ...
  router.post('/v21/feedback', function (req, res) {
    res.redirect('/v21/feedback')
  })


// v21 Check Your Answers

// N.B ALL PERSONALISATION VARIABLES NEED TO BE THERE, IF THEY'RE NOT REQUIRED YOU STILL NEED TO SEND AN EMPTY STRING ""

router.post('/v21/check-your-answers', function (req, res) {
  ...
  if (lastname == 'Green' || lastname == 'Elba') {
    res.redirect('/v21/apply/confirmation-pending-evidence')
    ...
    if (emailAddress) {
      ...
      if (pregnant === "yes") {
            res.redirect('/v21/apply/confirmation-successful');
      } else {
        res.redirect('/v21/apply/confirmation-successful');
      }
...
router.post('/v21/return-reference-number', function (req, res) {
  ...
  if (referencenumber) {
    ...
    res.redirect('/v21/apply/return-reference-number')
  }
...
router.post('/v21/return-date-of-birth', function (req, res) {
...
router.post('/v21/return-applicant-identity-select', function (req, res) {
...
router.post('/v21/online-account', function (req, res) {
  ...
  if (onlineAccount) {
    ...
    res.redirect('/v21/apply/online-account')
  }
...
router.post('/v21/email-address-2', function (req, res) {
