...
router.post('/v29/where-do-you-live', function (req, res) {


var location = req.session.data['location']

if (location === "england") {
  res.redirect('/v29/apply/name')
}
if (location === "wales") {
  res.redirect('/v29/apply/name')
}
...
if (location === "somewhere else") {
  res.redirect('/v29/apply/kickouts/not-eligible-country')
}
...
router.post('/v29/name', function (req, res) {

    var firstname = req.session.data['firstname']
    var lastname = req.session.data['lastname']

    res.redirect('/v29/apply/postcode')
  ...
  router.post('/v29/apply/postcode', function (req, res) {

    res.redirect('/v29/apply/address-2');
  ...
  router.post('/v29/apply/address-2', function (req, res) {
    ...
    res.redirect('/v29/apply/date-of-birth');
});
  ...
  router.post('/v29/date-of-birth', function (req, res) {
  ...
  if (yrs < 16) {
      res.redirect('/v29/apply/kickouts/confirmation-under-16');
  } else {
      res.redirect('/v29/apply/national-insurance-number');
  }
});
  ...
  router.post('/v29/national-insurance-number', function (req, res) {

    var nationalinsurancenumber = req.session.data['nationalinsurancenumber'].toUpperCase().replace(/\s+/g, '');

    res.redirect('/v29/apply/check-your-answers-personal-details')
    ...
    router.post('/v29/cya-personal-details', (req, res) => {
    //res.render('/v29/cya-personal-details', { data: req.session.data }); //Ensure Data is Properly Passed (it works without this)
    ...
    var yrs = moment().diff(moment(dobYrs), 'years'); // Calculate age
      ...
      if (firstname == 'RILEY' && lastname == 'JONES' && nationalinsurancenumber == 'CD654321B' && dateofbirth == '01/01/2000') {
        res.redirect('/v29/apply/kickouts/no-api-match-benefit')
      }
      else {
        res.redirect('/v29/apply/are-you-pregnant')
      }
  ...
  router.post('/v29/are-you-pregnant', function (req, res) {
    ...
    if (pregnant === "yes") {
      res.redirect('/v29/apply/due-date');
    }
    else if (pregnant === "no") {

      // NEW LOGIC: 16–17 year olds who are not pregnant
      if (yrs === 16 || yrs === 17) {
        return res.redirect('/v29/apply/kickouts/16-17-year-old-not-pregnant');
      }

      req.session.data.lessThanTenWeeksPregnant = true;
      res.redirect('/v29/apply/children-under-four');
    }
    else {
      res.redirect('/v29/apply/are-you-pregnant');
    }
  ...
  router.post('/v29/due-date', function (req, res) {
  ...
  router.post('/v29/children-under-four', function (req, res) {
    ...
    if (yrs >= 16 && yrs <= 17 && pregnant === "yes" && childrenunderfour === "yes") {
      return res.redirect('/v29/apply/16-17-cannot-receive-child-payments');
    }

    if (yrs >= 16 && yrs <= 17 && pregnant === "yes" && childrenunderfour === "no") {
      return res.redirect('/v29/apply/bank-details');
    }

    // Existing logic
    if (pregnant === "yes" && childrenunderfour === "no") {
      res.redirect('/v29/apply/email-address');
    } else if (pregnant === "no" && childrenunderfour === "yes") {
      res.redirect('/v29/apply/childs-first-name');
    } else if (pregnant === "yes" && childrenunderfour === "yes") {
      res.redirect('/v29/apply/childs-first-name');
    } else if (childrenunderfour === "no" && pregnant === "no") {
      res.redirect('/v29/apply/kickouts/not-pregnant-no-children');
    } else {
      res.redirect('/v29/apply/children-under-four');
    }
  ...
  router.post('/v29/bank-details', function (req, res) {

    res.redirect('/v29/apply/identity-evidence')
  ...
  router.post('/v29/childs-first-name', function (req, res) {
    ...
    if (childsfirstname && childslastname) {
      res.redirect('/v29/apply/childs-date-of-birth')
    }
    else {
      res.redirect('/v29/apply/childs-first-name')
    }
    ...
    router.post('/v29/childs-date-of-birth', function (req, res) {
      ...
      if (childsdateofbirthday && childsdateofbirthmonth && childsdateofbirthyear) {
        ...
        if (moment(childsdateofbirth).isBefore(today)) {
            ...
            if (moment(childsdateofbirth).isAfter(fouryearsfromtoday)) {
              ...
              res.redirect('/v29/apply/children-under-four-answers');
    ...
    router.post('/v29/children-under-four-answers', function (req, res) {
  ...
  router.post('/v29/contact-method', function (req, res) {
  ...
  router.post('/v29/mobile-phone-number', function (req, res) {

    var mobilePhoneNumber = req.session.data['mobilephonenumber']

    res.redirect('/v29/apply/check-your-answers')

  })

  // What is your mobile phone number?

  router.post('/v29/marketing-preference', function (req, res) {
...
router.post('/v29/check-your-answers', function (req, res) {
  ...
  if (age === 16 || age === 17) {
    return res.redirect('/v29/apply/confirmation-submitted');
  } else {
  res.redirect('/v29/apply/confirmation-successful');
  }
})
