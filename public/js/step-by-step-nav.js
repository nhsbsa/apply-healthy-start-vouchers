...
(function (Modules) {
  ...
  Modules.AppStepNav = function () {
    ...
    this.start = function ($element) {
      ...
      $element.addClass('app-step-nav--active'); // Prevent FOUC, remove class hiding content
      ...
      function addShowHideToggle() {
        $stepHeaders.each(function () {
          ...
          if (!$(this).find('.js-toggle-link').length) {
            $(this).find('.js-step-title-button').append('<span class="app-step-nav__toggle-link js-toggle-link" aria-hidden="true" hidden></span>');
          }
        });
      }
      ...
      function showPreviouslyOpenedSteps() {
        ...
        $.each($steps, function () {
          var id = $(this).attr('id');
          var stepView = new StepView($(this)); // show the step if it has been remembered or if it has the 'data-show' attribute
      ...
      function addButtonstoSteps() {
        $.each($steps, function () {
          ...
          $title.wrapInner('<button ' + 'class="app-step-nav__button app-step-nav__button--title js-step-title-button" ' + 'aria-expanded="false" aria-controls="' + contentId + '">' + '</button>');
        });
      }
      ...
      function ensureOnlyOneActiveLink() {
        ...
        var lastClicked = loadFromSessionStorage(sessionStoreLink) || $element.find('.' + activeLinkClass).first().attr('data-position'); // it's possible for the saved link position value to not match any of the currently duplicate highlighted links
      ...
      function setShowHideAllText() {
        var shownSteps = $element.find('.step-is-shown').length; // Find out if the number of is-opens == total number of steps
    ...
    function StepNavTracker(totalSteps, totalLinks, uniqueId) {
      this.track = function (category, action, options) {// noop
      };
    }
  };
